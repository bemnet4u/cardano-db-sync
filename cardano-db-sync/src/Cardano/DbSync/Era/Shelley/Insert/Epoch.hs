...
insertEpochStake _tracer icache epochNo stakeChunk = do
    ...
    lift $ DB.insertManyUncheckedUnique "Stakes" dbStakes
  where
    mkStake
...
insertRewards epoch icache rewardsChunk = do
    ...
    lift $ DB.insertManyUncheckedUnique "Rewards" dbRewards
  where
    mkRewards
        :: MonadBaseControl IO m
...
insertOrphanedRewards epoch icache orphanedRewardsChunk = do
    ...
    lift $ DB.insertManyUncheckedUnique "OrphanedRewards" dbRewards
  where
    mkOrphanedReward
        :: MonadBaseControl IO m
        => (Generic.StakeCred, Set (Shelley.Reward StandardCrypto))
